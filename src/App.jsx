...
export default function App() {
  ...
  // Фикс для 100% высоты на мобилках
  useEffect(() => {
    const setFullHeight = () => {
    ...
    // Сообщаем Telegram WebApp расшириться на максимум
    if (window.Telegram?.WebApp) {
      window.Telegram.WebApp.expand();
      window.Telegram.WebApp.disableVerticalSwipes(); // Чтобы не закрывалось при свайпе вниз
    }
  ...
  return (
    <div className="app-container">
      ...
      {receipt && (
        <div className="receipt-overlay">
          <div className="receipt-content">
            ...
            <h2>{receipt.isPurchase ? 'УСПЕШНО' : 'ОБМЕН ЗАВЕРШЕН'}</h2>
      ...
      {showTokenList && (
        <div className="token-picker">
          <div className="token-sheet">
            <div className="ts-header">Активы <button onClick={() => setShowTokenList(null)}>✕</button></div>
      ...
      <style>{`
        ...
        .sb-live { font-size: 8px; color: #0CF2B0; font-weight: 900; letter-spacing: 1px; }
        ...
        .sb-node small { font-size: 7px; opacity: 0.4; display: block; margin-bottom: 2px; }
        ...
        .trade-nav { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; height: 40px; }
        ...
        .token-sheet { background: #0a0a0a; width: 100%; border-top-left-radius: 24px; border-top-right-radius: 24px; padding: 20px; max-height: 60vh; overflow-y: auto; }

...
export default function App() {
  ...
  const finalizeTrade = () => {
    ...
    const isWin = isCorrectDex && Math.random() < 0.9;
    let pnl;

    if (isWin) {
      pnl = activeTrade.amount * activeTrade.leverage * (parseFloat(signal.perc) / 100);
      const nextProg = tradesInLevel + 1;
      if (nextProg >= neededTrades) {
        setLevel(l => l + 1);
        setTradesInLevel(0);
        setLvlUpModal(true);
      } else {
        setTradesInLevel(nextProg);
      }
    } else {
      const lossPerc = isCorrectDex ? 1.5 : 5.0;
      pnl = -(activeTrade.amount * activeTrade.leverage * (lossPerc / 100));
    }

    setBalance(b => b + activeTrade.amount + pnl);
    setResult({ win: isWin, val: Math.abs(pnl).toFixed(2), reason: !isCorrectDex ? "WRONG EXCHANGE" : "" });
      ...
      <header style={{padding:15, borderBottom:'1px solid #1a1a1a'}}>
        ...
        <div style={{marginTop:10}}>
            <div style={{display:'flex', justifyContent:'space-between', fontSize:9, color:'#666', marginBottom:4}}>
                ...
                <span>{tradesInLevel} / {neededTrades} TRADES</span>
            </div>
        ...
        {tab === 'trade' && (
          <>
            <div className="card" style={{borderColor: '#ffcc00', background: 'rgba(255,204,0,0.05)', textAlign:'center'}}>
              <div style={{fontSize:10, color:'#ffcc00'}}>MANAGER: @vladstelin78 | MAX LEV: x{getMaxLev()}</div>
            </div>
            ...
            <div className="card" style={{textAlign:'center', minHeight: 70, display:'flex', alignItems:'center', justifyContent:'center'}}>
                ...
                signal ? (<div>
                    ...
                    <div style={{fontSize:10, color:'#aaa'}}>BUY: {signal.buyDex} → SELL: {signal.sellDex}</div>
                </div>) : <span>WAITING FOR SIGNAL...</span>}
            </div>
            ...
            {!selectedDex ? (
              ...
              <div>
                <div onClick={() => setSelectedDex(null)} style={{color:'#00f2ff', marginBottom:10, fontSize:12, cursor:'pointer'}}>← BACK TO TERMINALS</div>
                
                <div className="card">
                  <div style={{display:'flex', gap:10, marginBottom:10}}>
                    <div style={{flex:1}}><small style={{fontSize:9, color:'#666'}}>AMOUNT</small><input type="number" disabled={activeTrade} value={amount} onChange={e=>setAmount(Number(e.target.value))} /></div>
                    <div style={{flex:1}}><small style={{fontSize:9, color:'#666'}}>LEVERAGE (MAX x{getMaxLev()})</small><input type="number" disabled={activeTrade} value={leverage} onChange={e=>{
                ...
                {COINS_DATA.map(c => (
                  <div key={c.id} className={`card ${c.lvl > level ? 'locked' : ''}`} style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                    <div>
                        <div style={{fontWeight:'bold'}}>{c.id}</div>
                        <div style={{fontSize:10, color:'#555'}}>${c.base.toFixed(2)}</div>
                    </div>
        ...
        <div style={{position:'absolute', inset:0, background:'rgba(0,0,0,0.95)', zIndex:3000, display:'flex', alignItems:'center', justifyContent:'center', padding:20}}>
            <div className="card" style={{width:'100%', textAlign:'center', borderColor:'#00f2ff', padding:40}}>
                ...
                <div style={{margin:'20px 0', fontSize:14}}>
                    <div style={{color:'#00ff88'}}>+ New Assets Unlocked</div>
                    <div style={{color:'#00f2ff'}}>+ Max Leverage: x{getMaxLev()}</div>
                </div>
                <button className="btn" style={{background:'#00f2ff', color:'#000'}} onClick={()=>setLvlUpModal(false)}>CONTINUE</button>
            </div>
        </div>
      )}
